...
*Proof.* Let P be any fixed n-cell one-sided polyhex. A container C for one-sided n-hexes must admit a placement of P, i.e. there exist a rotation rho and a translation t with (rho(P) + t) a subset of the cell set of C. Since |rho(P) + t| = |P| = n and all placed cells are distinct cells of C, we have |C| >= n. Minimising over containers yields a(n) >= n. QED.
...
| 7 | 21   | 8 x 5 | 1797.200        | PASS       | CERTIFIED (cadical --lrat + lrat-check) |

*Note on n=7 certification.* At n=7 the cadical-produced DRAT proof reached 1.1 GB (binary). drat-trim stalled at approximately 2 per cent of the file after 40 minutes of wall time on this machine, projected to 15-20 hours total. We re-emitted the UNSAT proof in LRAT format directly from cadical (the `--lrat=true` flag), producing a 3.37 GB LRAT certificate on the same CNF, and verified it with lrat-check in 80 seconds. cadical is the identical solver used in the main proof run, so the UNSAT verdict on the same CNF is the same mathematical event; only the proof-certificate format and checker differ. The n=1..6 DRAT certificates verified by drat-trim are preserved on disk alongside the LRAT certificate for n=7.
